...
4. **Logs** (stderr / terminal where `php -S` runs): expect `**camagru_notify_mail_attempt**` before delivery, then `**camagru_notify_mail_ok`** on a successful handoff to the local transport, or `**camagru_notify_mail_failed**` / `**camagru_notify_mail_exception**` when transport or `mail()` fails. Skipped cases (self-comment, preference off, missing config) are logged with `**camagru_notify_mail_skipped**`. **Seeing `mail_ok` means PHP passed the message to the local MTA**, not that the message reached an inbox (relay, DNS, spam filters, etc. can still block delivery).
