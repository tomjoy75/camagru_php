...
Ensure **unverified** accounts ( `email_verified = 0` after #47) **cannot** open an authenticated session: after **correct** email and password, login **succeeds only** if the user is **verified**. This closes the gap explicitly deferred from #47/#48.

## Behavior

- **Credential check unchanged up to password:** Email required; user lookup by email; `password_verify` as today. Wrong or missing credentials → same outcome as now (e.g. single generic **invalid email or password** message), **no** session.
...
- **Data source:** Use the **same** user row already returned for login (e.g. `findByEmail` including `email_verified`); no extra roundtrip unless the codebase already separates concerns and requires one.
...
- **S3:** Wrong password or unknown email still yields the **existing** generic invalid-credentials behavior, with **no** verification-specific wording.
- **S4:** No **500** responses on the normal login path for verified / unverified / invalid credential cases.

## Implementation Plan

1 keep AuthService::login email lookup and password_verify; wrong credentials return the existing generic form error and null user
...
- **S2:** After **GET** `/register/confirm?token=…` succeeds for that user (`email_verified = 1`), **POST** `/login` with the same credentials → **302** with `Location: /` (or equivalent success redirect) and a session cookie as today.

**Failure**

- **F1:** **POST** `/login` with **wrong password** for an existing account → **200**; body contains the existing generic **Invalid email or password** text; body **does not** contain the verification-only message.
- **F2:** **POST** `/login` with an **unknown** email → **200**; generic invalid-credentials copy only; **no** verification-only message.
...
curl -sS -o /tmp/cam49_wrong.body -w "F1 HTTP %{http_code}\n" -X POST "$BASE/login" \
  -d "email=$EMAIL" -d "password=Wrongpass999!"
grep -q 'Invalid email or password' /tmp/cam49_wrong.body && echo 'F1 OK'
...
curl -sS -o /tmp/cam49_unknown.body -w "F2 HTTP %{http_code}\n" -X POST "$BASE/login" \
  -d "email=nobody@example.com" -d "password=$PASS"
grep -q 'Invalid email or password' /tmp/cam49_unknown.body && echo 'F2 OK'
