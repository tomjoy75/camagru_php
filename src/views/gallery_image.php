<div class="space-y-4">
    <p>
        <a
            ...
            class="text-cyan-400 underline hover:text-cyan-300 text-sm"
        >← Back to gallery</a>
    </p>

    <?php if (!empty($detailLoadError)): ?>
        <p class="text-slate-400">This image could not be loaded. Please try again later.</p>
    <?php else: ?>
        <?php
        ...
        ?>
        <h1 class="text-xl font-semibold text-slate-100">Image</h1>
        ...
        <dl class="text-sm text-slate-300 space-y-1">
            <div><dt class="inline font-medium text-slate-100">Author:</dt>
                <dd class="inline ml-1"><?php echo htmlspecialchars($username ?? '', ENT_QUOTES, 'UTF-8'); ?></dd></div>
            <div><dt class="inline font-medium text-slate-100">Published:</dt>
                ...
                <div class="pt-2">
                    <form id="gallery-like-form" method="post" action="<?php echo htmlspecialchars('/gallery/like', ENT_QUOTES, 'UTF-8'); ?>" class="inline">
                        ...
                        <button
                            ...
                            class="text-sm px-3 py-1.5 rounded border border-slate-600 bg-slate-800 text-slate-100 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-slate-950"
                            aria-pressed="<?php echo !empty($hasLiked) ? 'true' : 'false'; ?>"
                        ><?php echo !empty($hasLiked) ? 'Unlike' : 'Like'; ?></button>
                    </form>
                </div>
            <?php endif; ?>
            <div><dt class="inline font-medium text-slate-100">Comments:</dt>
        ...
        <section class="mt-3" aria-label="Share this image">
            <h2 class="text-sm font-medium text-slate-100">Share</h2>
        ...
        <?php if (!empty($galleryCommentError)): ?>
            <p class="text-sm text-red-400 mt-2" role="alert"><?php echo htmlspecialchars((string) $galleryCommentError, ENT_QUOTES, 'UTF-8'); ?></p>
        <?php endif; ?>
        ...
        <?php if ($canInteract && $imgId >= 1): ?>
            ...
            >
                <input type="hidden" name="image_id" value="<?php echo $imgId; ?>">
                <label for="comment-content" class="block text-sm font-medium text-slate-200">Add a comment</label>
                <textarea
                    id="comment-content"
                    name="content"
                    rows="3"
                    class="w-full border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 bg-slate-800 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500"
                ></textarea>
                <button
                    type="submit"
                    class="text-sm px-3 py-1.5 rounded border border-slate-600 bg-slate-800 text-slate-100 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-slate-950"
        ...
        <section class="mt-6 border-t border-slate-700 pt-4" aria-label="Comments">
            ...
            <?php else: ?>
                <ul class="space-y-4 text-sm text-slate-300 list-none pl-0">
