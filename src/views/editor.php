...
<div class="w-full grid grid-cols-1 lg:grid-cols-5 gap-6">
    <section class="lg:col-span-4 space-y-4">
        <?php if (!empty($editorSuccess)): ?>
            <div class="rounded border border-emerald-600/80 bg-emerald-950/50 px-4 py-3 text-sm text-emerald-200">
                <?php echo htmlspecialchars($editorSuccess, ENT_QUOTES, 'UTF-8'); ?>
            </div>
        <?php endif; ?>

        <?php if (!empty($editorError)): ?>
            <div class="rounded border border-red-600/80 bg-red-950/40 px-4 py-3 text-sm text-red-200">
        ...
        <p
            ...
            class="rounded border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-300"
        ...
        <?php if ($isWorkspaceState): ?>
            <div id="editor-preview-host" class="bg-slate-800 rounded-lg aspect-video flex items-center justify-center text-slate-400 overflow-hidden ring-1 ring-slate-700/80">
                ...
                <?php else: ?>
                    <p class="text-slate-400">Workspace image unavailable.</p>
                <?php endif; ?>
            </div>
        <?php else: ?>
            <div id="editor-preview-host" class="bg-slate-800 rounded-lg aspect-video flex items-center justify-center text-slate-400 overflow-hidden ring-1 ring-slate-700/80">
                ...
                ></video>
                <p id="editor-webcam-fallback" class="text-slate-400">Webcam preview</p>
            </div>
        <?php endif; ?>
        ...
        <div class="bg-slate-900 rounded-lg border border-slate-700 p-4 shadow-md shadow-black/20">
                ...
                <form id="editor-compose-form" method="post" action="/editor/compose" class="space-y-3">
                    ...
                    <div class="flex flex-wrap gap-3">
                        <?php foreach ($stickers as $sticker): ?>
                            <button
                                type="button"
                                class="editor-sticker-pick inline-flex items-center justify-center w-16 h-16 rounded-md border border-slate-500/80 bg-slate-600 p-1 shrink-0 shadow-inner shadow-black/30 ring-1 ring-inset ring-white/5 hover:border-cyan-400/80 hover:bg-slate-500 hover:shadow-inner hover:shadow-cyan-950/20 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-slate-900"
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 sm:items-center text-sm text-slate-300">
            ...
            <?php if (isset($errors['compose'])): ?>
                <p class="mt-2 text-red-400 text-sm"><?php echo htmlspecialchars($errors['compose'], ENT_QUOTES, 'UTF-8'); ?></p>
            <?php endif; ?>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center flex-wrap">
            <form method="post" action="/editor/capture" id="editor-capture-form" class="flex flex-col gap-2">
                ...
                ?>
                <button type="submit" id="editor-capture-button" class="rounded bg-cyan-600 px-4 py-2 text-white font-medium hover:bg-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-slate-950<?php echo $editorCaptureDisabledNoSticker ? ' opacity-50 cursor-not-allowed' : ''; ?>"<?php echo $editorCaptureDisabledNoSticker ? ' disabled' : ''; ?>>
                    Capture
                </button>
            </form>
            <form method="post" action="/editor/upload" id="editor-upload-form" enctype="multipart/form-data" class="flex flex-col gap-2" data-editor-state="<?php echo htmlspecialchars($editorState, ENT_QUOTES, 'UTF-8'); ?>">
                ...
                <?php if (isset($errors['upload'])): ?>
                    <p class="text-red-400 text-sm"><?php echo htmlspecialchars($errors['upload'], ENT_QUOTES, 'UTF-8'); ?></p>
                <?php endif; ?>
                <div class="flex items-center">
                    <?php
                    $uploadLabelClasses = 'rounded border border-slate-600 bg-slate-800 px-4 py-2 text-slate-200 font-medium text-center';
            ...
            <?php if ($isWorkspaceState): ?>
                <form method="post" action="/editor/reset" class="flex flex-col gap-2">
                    <button type="submit" class="rounded border border-slate-600 bg-slate-800 px-4 py-2 text-slate-200 font-medium hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-slate-950">
    ...
    <aside class="lg:col-span-1">
        <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 sticky top-4 shadow-md shadow-black/20">
            <p class="text-sm font-medium text-slate-400 mb-3">Previous images</p>
            <div class="grid grid-cols-2 lg:grid-cols-1 gap-2">
                ...
                <?php else: ?>
                    <?php foreach ($savedImages as $saved): ?>
                        ...
                        ?>
                        <div class="space-y-2">
                            <a href="<?php echo htmlspecialchars($src, ENT_QUOTES, 'UTF-8'); ?>" class="aspect-square bg-slate-800 rounded overflow-hidden block border border-slate-700 hover:border-cyan-500/50 focus:outline-none focus:ring-2 focus:ring-cyan-500">
                            ...
                            <?php if ($imageId !== null): ?>
                                <form method="post" action="/editor/delete">
                                    ...
                                    <button type="submit" class="rounded bg-red-600 px-2 py-1 text-white text-xs hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2 focus:ring-offset-slate-900">
                                        Delete
                                    </button>
                                </form>
                            <?php endif; ?>
