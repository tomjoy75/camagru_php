...
class RegistrationConfirmationMailService
{
    ...
    /**
     * Send one plain-text email with absolute password-reset URL (phase 2 consumes token). Never throws.
     */
    public static function trySendPasswordResetRequest(string $recipientEmail, string $plainToken): void
    {
        try {
            $baseUrl = MailEnv::validatedAppBaseUrl();
            if ($baseUrl === null) {
            ...
            $resetUrl = $baseUrl . '/password-reset/confirm?token=' . rawurlencode($plainToken);
            ...
            $mailOk = @mail($to, $subject, $body, $headers);
            if ($mailOk) {
                error_log('camagru_password_reset_mail_ok');
            } else {
                error_log('camagru_password_reset_mail_failed');
            }
        } catch (Throwable $e) {
            error_log('camagru_password_reset_mail_exception');
        }
    }
}
